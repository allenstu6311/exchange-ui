...
export function getCurrentPrice(formData: IExForm, lastPrice: string, isMarket: boolean) {
...
export function handlePriceInput({
    ...
    if (currPrice && formData.quantity) {
        // 計算成交額 => 金額 * 數量
        const newAmount = mul(currPrice, formData.quantity, { precision: quoteAssetPrecision })
        setAmount(newAmount);
...
export function handleAmountInput({
    ...
    if (currPrice && value) {
        ...
        formData.price = div(value, formData.quantity, { precision: showPrecision })
    } else {
        formData.quantity = '';
    }
    setAmount(value)
...
export function handleSilderInput({
    ...
    const newAmount = div(mul(assets, value), 100, { precision: quoteAssetPrecision });
    setAmount(newAmount);

    if (currPrice) {
        formData.quantity = div(newAmount, currPrice, { precision: tradePrecision });
    }
}